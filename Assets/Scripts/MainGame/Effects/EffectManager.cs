...
public class EffectManager : MonoBehaviour
{
	...
	int EffectSum(Func<Effect, int?> val, int def, int min = int.MinValue) => Mathf.Max(
		// minimum value allows
		min,
		// default value
		def +
		// sum up valid values
	...
	public int Actions => EffectSum(ef => ef.ActionBonus, 2, 0);

	public bool Traversable => EffectAnd(ef => ef.Traversable);

	public int MoveRange => EffectSum(ef => ef.MoveRangeBonus, 1, 0);

	public bool CanMove => EffectAnd(ef => ef.CanMove);

	public int LevelCap => EffectSum(ef => ef.LevelCapBonus, 5, 1);
